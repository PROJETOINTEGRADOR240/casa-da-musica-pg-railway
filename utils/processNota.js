...
async function processNota(nota) {
    try {
        const [alunoResult] = await pool.query('SELECT nome FROM alunos WHERE idaluno = ?', [nota.aluno_id]);
        const [professorResult] = await pool.query('SELECT nome FROM professores WHERE idprofessor = ?', [nota.professor_id]);
        const [disciplinaResult] = await pool.query('SELECT nome FROM disciplinas WHERE iddisciplina = ?', [nota.disciplina_id]);
